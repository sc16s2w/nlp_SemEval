...
class OffensiveProcessor_task1(DataProcessor):
...
class OffensiveProcessor_task2(DataProcessor):
    """Processor for the MRPC data set (GLUE version)."""
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_test_examples(
            self._read_tsv(os.path.join(data_dir, "test_task3.txt")), "test")
...
class OffensiveProcessor_task3(DataProcessor):
    """Processor for the MRPC data set (GLUE version)."""
...
def main():
    ...
    parser.add_argument("--do_test",
                        action='store_true',
                        help="Whether to run test on the tset set.")
    ...
    processors = {
        "one": OffensiveProcessor_task1,
        "two": OffensiveProcessor_task2,
        "three": OffensiveProcessor_task3,
    }
    ...
    if args.do_train:
        ...
        # Prepare model
        model = BertForSequenceClassification.from_pretrained(args.bert_model, num_labels)
        if args.fp16:
            model.half()
        model.to(device)
